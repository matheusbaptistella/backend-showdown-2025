...
#[derive(Clone)]
pub struct Worker {
    cmd_queue_rx: Arc<Mutex<mpsc::Receiver<Command>>>,
...
impl Worker {
    pub fn new(tx: mpsc::Sender<Command>, rx: mpsc::Receiver<Command>) -> Self {
        Worker {
            cmd_queue_rx: Arc::new(Mutex::new(rx)),
            cmd_queue_tx: tx,
    ...
    pub async fn run(&self) {
        loop {
            let maybe_cmd = {
                let mut guard = self.cmd_queue_rx.lock().await;
                guard.recv().await
            };
    ...
    async fn process(&self, cmd: Command) {
        match cmd {
            ...
            Command::Set(p) => {
                ...
                let status = self.http.post(url).json(&p).send().await.unwrap().status();
                ...
                if status.is_success() {
                    ...
                    match processor {
                        Processor::Default => self.default_db.set(timestamp, amount),
                        Processor::Fallback => self.fallback_db.set(timestamp, amount),
                    }
    ...
    fn local_summary(
        ...
        let (d_count, d_total) = self.default_db.get(from, to);
        let (f_count, f_total) = self.fallback_db.get(from, to);
    ...
    async fn remote_summary(
        ...
        let resp = self.http.get(endpoint).query(&params).send().await.unwrap();
