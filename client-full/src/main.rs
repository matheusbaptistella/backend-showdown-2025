use std::sync::Arc;
use client_full::{CreatePayment, PaymentProcessorsSummaries, PaymentsSummaryQueryParams, RequestPayment, Summary, Db, DbHandle};
...
async fn process_payment(job: RequestPayment, processor: Processor, state: &AppState) {
    let mut url = "http://payment-processor-default:8080/payments";
    let mut retry = false;
    ...
    loop {
        ...
        if resp.status().is_server_error() {
            retry = true;
            url = match processor {
                Processor::Default => "http://payment-processor-fallback:8080/payments",
                Processor::Fallback => "http://payment-processor-default:8080/payments",
            };
