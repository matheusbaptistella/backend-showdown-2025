...
To get things started our objective will be to deploy one instance of the Rust backend with a Redis instance and an Nginx load-balancer. Also, we will limit ourselves to using alway the default Payment Processor instance. Clearly not the best scenario but we want to track our improvement.
...
### v1.0
To start off we created a version of the backend which consistend of a single instance of our server, the Nginx load balaner and the Redis cache. After runnig the default tests locally, some charts stand out:

![Fail Rate and Waiting Time for version v1](images/v1/fail-and-waiting-rates.png)

As we can see, at some point our backend can't handle any of the incoming requests and they fail with a timeut of 1.5s.

This is the first working version of the backend. It consists of a single backend instance capable of making requests only to the `default` payment processor instance. We also use Redis to store the information for the summaries.